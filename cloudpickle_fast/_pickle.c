    ...
    if (state) {
        if (state_setter == NULL) {
            ...
            if PyDict_Check(state)
                statetup = Py_BuildValue("(OOO)", state_setter, state, Py_None);
            else if PyTuple_Check(state) {
                if (PyTuple_GET_SIZE(state) == 2){
                    statetup = Py_BuildValue("(OOO)",
                                             (PyObject *)state_setter,
                                             (PyObject *)PyTuple_GetItem(state, 0),
                                             (PyObject *)PyTuple_GetItem(state, 1));
                }
            }

            if (statetup == NULL) {
                PyErr_SetString(st->PicklingError,
                                "state should be either a dict or a tuple of"
                                " length 2");
                return -1;
            }
...
load_build(UnpicklerObject *self)
{
    ...
    /* proto 5 addition: state can embed a callable state setter */
        ...
        if (PyObject_CallFunctionObjArgs(setstate, inst, state,
                                         slotstate, NULL) == NULL){
            PickleState *st = _Pickle_GetGlobalState();
            PyErr_SetString(st->UnpicklingError, "try to setstate failed ");
            return -1;
        }
