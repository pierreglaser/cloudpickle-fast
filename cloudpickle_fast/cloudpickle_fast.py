...
from cloudpickle_fast._pickle import Pickler
...
def dynamic_class_reconstructor(tp, name, bases, tp_kwargs, registered_subclasses):
...
def function_setstate(obj, state, slotstate):
    ...
    # closure and globals are readonly attributes, we have to modify them
    # inplace.
    obj_globals = slotstate.pop("globals")
    obj_closure = slotstate.pop("closure")

    obj.__globals__.update(obj_globals)

    for i, cell in enumerate(obj_closure):
        obj.__closure__[i].cell_contents = cell.cell_contents

    for k, v in slotstate:
        print(k)
        setattr(obj, k, v)
